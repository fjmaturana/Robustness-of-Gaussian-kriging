"""Numerical experiments for "Exact and Local Robustness of Gaussian
Kriging under Non-Gaussian Dependence".
...
def mixture_slope_figure(
    ...
    The fit uses the same eligibility rule as ``estimate_local_slope``. Error
    bars are paired Monte Carlo standard errors of the excess-risk estimates.
    The shaded region is the pointwise 95% confidence band for the mean fitted
    log excess risk under the descriptive unweighted OLS regression.
    """
    import matplotlib
    ...
    if len(eligible) < 3:
        raise ValueError("At least three eligible mixture points are required")

    fractions = np.asarray(
        [row["eta_fraction"] for row in eligible], dtype=float
    )
    excess = np.asarray([row["excess"] for row in eligible], dtype=float)
    excess_mcse = np.asarray(
        [row["mcse_excess"] for row in eligible], dtype=float
    )
    eta = np.asarray([row["eta"] for row in eligible], dtype=float)
    ...
    eta_max = float(eligible[0]["eta_max"])
    ...
    degrees_of_freedom = len(eligible) - 2
    ...
    axis.errorbar(
        ...
        elinewidth=0.9,
        capsize=2.5,
        label="Simulated excess risk (paired MCSE)",
        zorder=3,
    )
    axis.set_xscale("log")
    axis.set_yscale("log")
...
def latex_mixture_figure(
    ...
    caption = (
        ...
        "$\\widetilde b=(1,-1,0,\\ldots,0)^\\top$. Error bars are "
        f"paired Monte Carlo standard errors based on $N={n_sim:,}$ "
        "replicates. The descriptive unweighted log--log regression has "
        ...
        "The shaded region is the pointwise $95\\%$ confidence band for "
        "the mean fitted log excess risk; it does not include Monte Carlo "
